...
def leer_posiciones_entrada():
    while True:
        ...
        # Validar el formato y unicidad de cada posición
        posiciones_validas = []
        ...
        # Mostrar errores si hay posiciones inválidas o repetidas
        ...
        # Si todas las posiciones son válidas y hay al menos una, retornarlas
...
def es_ataque_valido(origen, destino, posiciones_dict):
    ...
    # Misma posición, no es ataque válido
    ...
    # Verificar que no haya piezas intermedias
    if delta_fila == 0:  # Misma fila
        col = col_o + paso_col
        while col != col_d:
            # Verificar si hay alguna reina en esta posición
...
def mostrar_tablero(posiciones_dict):
    ...
    for reina, pos in posiciones_dict.items():
        ...
        # En la representación del tablero, la fila 8 está arriba (índice 0)
        # y la fila 1 está abajo (índice 7)
        fila_tablero = 7 - fila
    ...
    # Imprimir el tablero y devolverlo como string
    print(tablero_str)
    return tablero_str
    ...
    while ciclo_sin_ataques < len(reinas_vivas) and len(reinas_vivas) > 1:
        ...
        for reina_objetivo in reinas_vivas:
            ...
            if es_ataque_valido(pos_actual, pos_objetivo, posiciones):
                ...
                # Actualizar posición de la reina actual y eliminar la objetivo
